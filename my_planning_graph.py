...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            ...
            if anode.prenodes.issubset(self.s_levels[level]):
                ...
                # connect to s_node in previous levels
    ...
    def add_literal_level(self, level):
        ...
        for anode in self.a_levels[level-1]:
            for effnode in anode.effnodes:
                if effnode in snode_dict:
                    effnode = snode_dict[effnode]
                else:
                    snode_dict[effnode] = effnode
                effnode.parents.add(anode)
                anode.children.add(effnode)
        self.s_levels.append(set(snode_dict))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # True if positive effect of node1 overlap w/ negative effect of node2
        # True if negative effect of node1 overlap w/ positive effect of node2
        return not set(node_a1.action.effect_add).isdisjoint(
                set(node_a2.action.effect_rem)) or \
           not set(node_a1.action.effect_rem).isdisjoint(
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return not set(node_a1.action.precond_pos).isdisjoint(
                set(node_a2.action.effect_rem)) or \
            not set(node_a1.action.precond_neg).isdisjoint(
                set(node_a2.action.effect_add)) or \
            not set(node_a2.action.precond_pos).isdisjoint(
                set(node_a1.action.effect_rem)) or \
            not set(node_a2.action.precond_neg).isdisjoint(
                set(node_a1.action.effect_add))
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            # find level cost: the first State Level that goal appears
            level_cost = 0
            goal_node = PgNode_s(g, True)
            while goal_node not in self.s_levels[level_cost]:
                level_cost += 1
                if level_cost >= num_levels:
                    raise ValueError("Problem is not solluble")
